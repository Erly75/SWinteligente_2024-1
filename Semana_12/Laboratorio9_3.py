...
imagen = cv2.imread("1a.jpg") ## leemos la imagen a trasladar
...
print(alto)
tx = 13 ##definimos la traslación en x
ty = 5 ## definimos la traslación en y

mT = np.array(([1,0,tx],[0,1,ty],[0,0,1]),np.uint8) ##creamos nuestra matríz de transformación

imgT = np.zeros((alto+5,ancho+13,3),np.uint8) ##generamos una imagen base donde se guardará la imagen trasladada
